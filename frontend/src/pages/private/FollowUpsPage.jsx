    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <PanelCard className="xl:col-span-2">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <SelectionStateCard
            emptyText="Cuando selecciones un seguimiento desde la tabla, vas a ver su resumen operativo acá."
            lines={
              selectedFollowUp
                ? [
                    `Caso: ${selectedFollowUp.child.firstName} ${selectedFollowUp.child.lastName}`,
                    `Fecha: ${formatDate(selectedFollowUp.followUpDate ?? selectedFollowUp.session?.startsAt ?? selectedFollowUp.createdAt)}`,
                    `Resumen: ${selectedFollowUp.summary || selectedFollowUp.title || 'Sin resumen breve'}`,
                  ]
                : []
            }
            title={selectedFollowUp?.title || selectedFollowUp?.summary || 'Seguimiento en edición'}
          />
        </div>
        ...
        <form className="mt-6 grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
          ...
          {error ? <FormErrorAlert className="md:col-span-2">{error}</FormErrorAlert> : null}
